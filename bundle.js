...
/***/ function(module, exports, __webpack_require__) {
	...
	var paperMixingBowl = __webpack_require__(36);
	var paperGrill = __webpack_require__(37);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onShowCP = function onShowCP() {
	  $('.first-window').addClass('hidden');
...
/***/ function(module, exports) {
	...
	var app = {
	  host: 'https://recipehouse.herokuapp.com'
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var stepCountMake = 1;
	var onAddStepsMake = function onAddStepsMake() {
	  var stepNumberMake = stepCountMake.toString();
	...
	var ingredientCountMake = 1;
	var onAddIngredientsMake = function onAddIngredientsMake() {
	  var ingredientNumberMake = ingredientCountMake.toString();
	...
	var stepCountEdit = 1;
	var onAddStepsEdit = function onAddStepsEdit() {
	  var stepNumberEdit = stepCountEdit.toString();
	...
	var ingredientCountEdit = 1;
	var onAddIngredientsEdit = function onAddIngredientsEdit() {
	  var ingredientNumberEdit = ingredientCountEdit.toString();
	...
	var convertObjectToArry = function convertObjectToArry(obj) {
	...
	var onCreateRecipe = function onCreateRecipe(event) {
	  ...
	  api.createRecipe(data).done(ui.addRecipeSuccess).fail(ui.failure);
	};
	...
	var onEditRecipe = function onEditRecipe(event) {
	  ...
	  data.recipe.instructions = convertObjectToArry(data.recipe.instructions);
	  data.recipe.ingredients = convertObjectToArry(data.recipe.ingredients);
	  var allUserRecipes = ui.getRecipeInfo();
	  for (var i = 0; i < allUserRecipes.length; i++) {
	    var userRecipe = allUserRecipes[i];
	    if (userRecipe.name === data.recipe.name) {
	      console.log(userRecipe);
	      api.editRecipe(data, userRecipe.id).done(ui.createRecipeSuccess).fail(ui.failure);
	    }
	  }
	};

	var onUserRecipe = function onUserRecipe() {
	  ...
	  api.getAllRecipes().done(ui.getUserRecipeSuccess).fail(ui.failure);
	};

	var onHomeRecipe = function onHomeRecipe(event) {
	  event.preventDefault();
	  // $('.tab-content').empty("");
	  $('.recipe-content').removeClass('hidden');
	  ...
	  api.getAllRecipes().done(ui.getHomeRecipeSuccess).fail(ui.failure);
	};

	var onFavoriteRecipe = function onFavoriteRecipe(event) {
	  event.preventDefault();
	  $('.favorite-recipe-content').text("");
	  api.getLikeRecipe().done(ui.getLikeRecipeSuccess).fail(ui.failure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var editRecipe = function editRecipe(data, id) {
	  return $.ajax({
	...
	var getLikeRecipe = function getLikeRecipe() {
	  return $.ajax({
...
/***/ function(module, exports, __webpack_require__) {
	...
	var app = __webpack_require__(6);
	var extraRecipeApi = __webpack_require__(12);
	var api = __webpack_require__(10);
	...
	var addRecipeSuccess = function addRecipeSuccess(data) {
	  ...
	  api.getAllRecipes().done(getUserRecipeSuccess).fail(failure);
	};
	...
	var delRecipeSuccess = function delRecipeSuccess() {
	  console.log("success!");
	  api.getAllRecipes().done(getUserRecipeSuccess).fail(failure);
	};

	var favorRecipeSuccess = function favorRecipeSuccess(data) {
	...
	var addLike = function addLike(data) {
	  $('#add-like-' + data.id.toString()).on('click', function () {
	    var recipeId = $('#like-recipe-' + data.id.toString()).val();
	    extraRecipeApi.addLikeRecipe(recipeId).done(favorRecipeSuccess).fail(failure);
	  });
	};

	var addDel = function addDel(data) {
	  $('#del-recipe-' + data.id.toString()).on('click', function () {
	    var recipeId = $('#delete-recipe-' + data.id.toString()).val();
	    extraRecipeApi.deleteRecipe(recipeId).done(delRecipeSuccess).fail(failure);
	  });
	};

	var switchCategoryOnHome = function switchCategoryOnHome(each) {
	  var homeRecipeListing = __webpack_require__(13);
	...
	var switchCategoryOnUser = function switchCategoryOnUser(each) {
	  var userRecipeListing = __webpack_require__(33);
	  switch (each.category.toLowerCase()) {
	    case "grill":
	      console.log(each.category);
	...
	var getUserRecipeSuccess = function getUserRecipeSuccess(data) {
	  var editRecipeListing = __webpack_require__(34);
	  $('.home-content').text("");
	  $('.edit-field').text("");
	  var recipeArr = data.recipes;
	  for (var i = 0; i < recipeArr.length; i++) {
	    var each = recipeArr[i];
	...
	var getHomeRecipeSuccess = function getHomeRecipeSuccess(data) {
	  $('.home-content').text("");
	  var recipeArr = data.recipes;
	  for (var i = 0; i < recipeArr.length; i++) {
	    var each = recipeArr[i];
	...
	var getLikeRecipeSuccess = function getLikeRecipeSuccess(data) {
	  var recipeArr = data.like_recipes;
	  for (var i = 0; i < recipeArr.length; i++) {
	    var each = recipeArr[i];
	    if (each.user_id === app.user.id) {
	      console.log(each.recipe_id);
	      extraRecipeApi.getLikeRecipes(each.recipe_id).done(likeRecipeSuccess).fail(failure);
	    }
	  }
	};

	var likeRecipeSuccess = function likeRecipeSuccess(data) {
	  var likeRecipeListing = __webpack_require__(35);
	  console.log(data);
	...
	module.exports = {
	  ...
	  getUserRecipeSuccess: getUserRecipeSuccess,
	  getHomeRecipeSuccess: getHomeRecipeSuccess,
	  getLikeRecipeSuccess: getLikeRecipeSuccess,
	  likeRecipeSuccess: likeRecipeSuccess,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var addLikeRecipe = function addLikeRecipe(recipeId) {
	  return $.ajax({
	    url: app.host + '/like_recipes',
	    method: "POST",
	    data: {
	...
	var getLikeRecipes = function getLikeRecipes(recipeId) {
	  return $.ajax({
	...
	module.exports = {
	  addLikeRecipe: addLikeRecipe,
	  getLikeRecipes: getLikeRecipes,
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div class=\"container col-md-4\">\n    <div class=\"panel panel-default\">\n        <div class=\"panel-heading\">\n            <div class=\"btn-group\" style=\"float:left;\">\n              <h4 class=\"media-heading\">Recipe Info</h4>\n            </div>\n            <div class=\"btn-group\" style=\"float:right;\">\n              <form id=\"add-recipe-info\">\n                <fieldset>\n                  <input id=\"like-recipe-"
	    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
	    + "\" type=\"hidden\" name=\"like_recipe[recipe_id]\" value="
	    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
	    + ">\n                  <span id=\"add-like-"
	    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
	    ...
	    + "                   </ul>\n                  </div>\n                  <div>\n                    <p>Instructions: </p>\n                    <ul>\n"
	    + ((stack1 = helpers.each.call(alias1,(depth0 != null ? depth0.instructions : depth0),{"name":"each","hash":{},"fn":container.program(3, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "")
	    + "                   </ul>\n                  </div>\n                <div class=\"clearfix\"></div>\n               </div>\n            </div>\n        </div>\n    </div>\n</div>\n";
	},"useData":true});
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div class=\"container col-md-4\">\n    <div class=\"panel panel-default\">\n        <div class=\"panel-heading\">\n            <div class=\"btn-group\" style=\"float:left;\">\n              <h4 class=\"media-heading\">"
	    + alias4(((helper = (helper = helpers.name || (depth0 != null ? depth0.name : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"name","hash":{},"data":data}) : helper)))
	    + "</h4>\n            </div>\n            <div class=\"btn-group\" style=\"float:right;\">\n              <form id=\"del-recipe-info\">\n                <fieldset>\n                  <input id=\"delete-recipe-"
	    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
	    + "\" type=\"hidden\" name=\"recipe[id]\" value="
	    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
	    + ">\n                  <button id=\"del-recipe-"
	    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
...
/***/ function(module, exports) {
	...
	var paperMixing = function paperMixing() {
	  ...
	  var spine = new Path({ x: view.center.x - 50, y: view.center.y - 40 });
	  ...
	  view.onFrame = function (event) {
	    for (var i = 1; i <= amount; i++) {
	      ...
	      var _sinus = Math.sin(event.time * 4 + i);
	      segment.point.y = _sinus * height + 105;
	    };
...
/***/ function(module, exports) {
	...
	var paperGrill = function paperGrill() {
	  var canvas = document.getElementById('myCanvasGrill');
	  paper.setup('myCanvasGrill');
	  var unit = 50;
	  var bottomPlate = new Path(new Point(3 * unit, 5 * unit), new Point(5 * unit, 5 * unit), new Point(7 * unit, 5 * unit));
	  bottomPlate.strokeColor = 'black';

	  //drawing bottom curve
	  var center1 = new Point(5 * unit, 5 * unit);
	  var vector = new Point(2 * unit, 0);
	  var bottomCurve = new Path();
	  vector.angle = -1;
	  for (var i = 0; i <= 180; i++) {
	    vector.angle += 1;
	    var pointOnCurve = center1 + vector;
	  ...
	  var handlebarRight = new Path();
	  var vecHandleRight = new Point(2 * unit, 0);
	  vecHandleRight.angle += 5;
	  var handlePoint1 = center1 + vecHandleRight;
	  handlebarRight.add(handlePoint1);
	  var handlePoint2 = handlePoint1 + [.3 * unit, 0];
	  handlebarRight.add(handlePoint2);
	  handlebarRight.strokeColor = 'black';
	  var nobRight = new Path.Circle(handlePoint2, .1 * unit);
	  nobRight.fillColor = 'black';

	  //Drawing handle bar on left
	  var handlebarLeft = new Path();
	  var vecHandleLeft = new Point(2 * unit, 0);
	  vecHandleLeft.angle += 175;
	  var handlePoint3 = center1 + vecHandleLeft;
	  handlebarLeft.add(handlePoint3);
	  var handlePoint4 = handlePoint3 + [-.3 * unit, 0];
	  handlebarLeft.add(handlePoint4);
	  handlebarLeft.strokeColor = 'black';
	  var nobLeft = new Path.Circle(handlePoint4, .1 * unit);
	  nobLeft.fillColor = 'black';

	  //Drawing stand on right
	  var standRight = new Path();
	  var vecStandRight = new Point(2 * unit, 0);
	  vecStandRight.angle += 60;
	  var standPoint1 = center1 + vecStandRight;
	  standRight.add(standPoint1);
	  var vecStandDirection1 = new Point(2 * unit, 0);
	  vecStandDirection1.angle += 60;
	  var standPoint2 = standPoint1 + vecStandDirection1;
	  standRight.add(standPoint2);
	  standRight.strokeColor = 'black';

	  //Drawing stand on left
	  var standLeft = new Path();
	  var vecStandLeft = new Point(2 * unit, 0);
	  vecStandLeft.angle += 120;
	  var standPoint3 = center1 + vecStandLeft;
	  standLeft.add(standPoint3);
	  var vecStandDirection2 = new Point(2 * unit, 0);
	  vecStandDirection2.angle += 120;
	  var standPoint4 = standPoint3 + vecStandDirection2;
	  standLeft.add(standPoint4);
	  standLeft.strokeColor = 'black';

	  //Drawing roller on left stand
	  var rollerCenter = standPoint4 - vecStandDirection2 / 2 * .3;
	  var roller = new Path.Circle(rollerCenter, .3 * unit);
	  var rollerCopy = roller.clone();
	  var rollerCC = roller.clone();
	  rollerCopy.scale(.7);
	  ...
	  //racks
	  var rack = new Path.Rectangle(0, 0, .3 * unit, .1 * unit);
	  rack.fillColor = 'black';

	  var rackSymbol = new Symbol(rack);

	  for (var _i = 0; _i < 7; _i++) {
	    var rackPosition = new Point((3.5 + _i / 2) * unit, 4.9 * unit);
	    rackSymbol.place(rackPosition);
	  }

	  //draw steak on rack
	  var steak = new Path.Rectangle(3.7 * unit, 4.65 * unit, 2.5 * unit, 0.2 * unit);
	  steak.fillColor = '#ff7777';

	  //Drawing hanging bar between stands
	  var bar = new Path();
	  var barPointRight = standPoint1 + vecStandRight / 2 * 0.9;
	  var barPointLeft = standPoint3 + vecStandLeft / 2 * 0.9;
	  bar.add(barPointRight, barPointLeft);
	  bar.strokeColor = 'black';

	  //Drawing top plate
	  var topPlate = new Path();
	  var topVector = new Point(2 * unit, 0);
	  topVector.angle = -145;
	  var topPlateRightPoint = new Point(7 * unit, 5 * unit);
	  var topPlateLeftPoint = topPlateRightPoint + topVector * 2;
	  topPlate.add(topPlateRightPoint, topPlateLeftPoint);
	  topPlate.strokeColor = 'black';

	  //drawing top curve
	  var center2 = topPlateRightPoint + topVector;
	  var topCurve = new Path();
	  var topVectorCopy = topVector.clone();
	  topVector.angle += -1;
	  for (var _i2 = 0; _i2 <= 180; _i2++) {
	    topVector.angle += 1;
	    var pointOnTopCurve = center2 + topVector;
	    topCurve.add(pointOnTopCurve);
	  }
	  topCurve.strokeColor = 'black';
	  topCurve.smooth();
	  var topCurveCopy = topCurve.clone();
	  ...
	  var stem = new Path();
	  var middlePoint = center2 + topVectorCopy;
	  var stemPoint = middlePoint + topVectorCopy / 2 * 0.3;
	  stem.add(middlePoint, stemPoint);
	  stem.strokeColor = 'black';
	  stem.strokeWidth = 0.05 * unit;
	  var barSymbol = rackSymbol.place(stemPoint).rotate(35).scale(1.5);

	  //joint roller for top and bottom curve
	  var joint = new Path.Circle(topPlateRightPoint, 0.15 * unit);
	  ...
	  for (var _i3 = 0; _i3 <= amount; _i3++) {
	    smokePath.add(new Point(1, _i3 / amount) * unit);
	  }
	  smokePath.selected = true;
	  var smokeSymbol = new Symbol(smokePath);
	  for (var _i4 = 0; _i4 < 7; _i4++) {
	    var smokeSize = Math.random() * 2;
	    var smokeCenter = new Point(_i4 / 2 * unit, 4.9 * unit);
	    var placedSymbol = smokeSymbol.place(smokeCenter - [6.5 * unit, 0.9 * unit]);
	    // 	placedSymbol.scale(smokeSize);
	  }

	  //group top cover elements
	  var topCoverGroup = new Group([topPlate, topCurve, topCurveCopy, stem, barSymbol, joint]);
	  topCoverGroup.fillColor = 'white';

	  var rotatingAngle = 0.1;
	  function onFrame(event) {
	    ...
	    // grilling smoke
	    for (var _i5 = 0; _i5 <= amount; _i5++) {
	      var segment = smokePath.segments[_i5];
	      var sinus = Math.sin(event.time * 5 + _i5);
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function(global) {module.exports = global["paper"] = __webpack_require__(51);
	/* WEBPACK VAR INJECTION */}.call(exports, (function() { return this; }())))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var window = self ? self.window : __webpack_require__(52),
